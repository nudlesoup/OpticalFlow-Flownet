...
def writeXYflow(filename, uv, v=None):
	""" Write optical flow to file.
	...
	dx = filename[:index] + '-dx' + filename[index:]
	dy = filename[:index] + '-dy' + filename[index:]
	x = open(dx, 'wb')
	y = open(dy, 'wb')
	# write the header
	x.write(TAG_CHAR)
	...
	x.close()
	y.close()
	np.savetxt(dx,u.astype(np.float32))
	np.savetxt(dy,v.astype(np.float32))
