        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            if args.save_flow or args.render_validation:
                for i in range(args.inference_batch_size):
                    ...
                    flow_utils.writeFlow( join(flow_folder, '{}-%06d.flo'.format(filename)%(batch_idx * args.inference_batch_size + i)),  _pflow)


                    # You can comment out the plt block in visulize_flow_file() for real-time visualization
                    if args.inference_visualize:
                        flow_utils.visulize_flow_file(
                            join(flow_folder, '{}-%06d.flo'.format(filename) % (batch_idx * args.inference_batch_size + i)),flow_vis_folder)
