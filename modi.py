        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            if args.save_flow or args.render_validation:
                for i in range(args.inference_batch_size):
                    ...
                    u = _pflow[:, :, 0]
                    v = _pflow[:, :, 1]
                    Image.fromarray(u).save( join(flow_folder, 'flow_x_%04d.jpg'.format(filename) % (batch_idx * args.inference_batch_size + i)))
                    Image.fromarray(v).save(join(flow_folder, 'flow_y_%04d.jpg'.format(filename) % (batch_idx * args.inference_batch_size + i)))
