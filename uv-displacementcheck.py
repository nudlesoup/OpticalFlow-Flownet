...
if __name__ == '__main__':
    refFilename = "/home/nudlesoup/Research/flownet2-pytorch/rangetest/kdata/imxx493-big.png"
    # print("Reading reference image : ", refFilename)
    img1 = cv2.imread(refFilename, cv2.IMREAD_COLOR)

    # Read image to be aligned
    imFilename = "/home/nudlesoup/Research/flownet2-pytorch/rangetest/kdata/imxx494-big.png"
    ...
    #H = homography2("/home/nudlesoup/Research/flownet2-pytorch/example/2/000000.flo")
    flow_data = readFlow("/home/nudlesoup/Research/flownet2-pytorch/rangetest/kdata/K.flo")
    ...
    pr1 = np.where(abs(u) < 50)
    pr2 = np.where(abs(u) < 50)
    print(len(pr1[0]))
    print(len(pr2[0]))
    for i in range(len(corners1)):
        pt2 = np.array([corners1[i][0], corners1[i][1], 1])
        # pt1 = pt1.reshape(3, 1)
        # pt2 = np.dot(H, pt1)
        # pt2 = pt2 / pt2[2]
        #print(pt2)
        pt2[0]= u[corners1[i][0], corners1[i][1]]
        pt2[1]=v[corners1[i][0], corners1[i][1]]
        # print(pt2)
        # print(u[corners1[i][0], corners1[i][1]])
        # print(v[corners1[i][0], corners1[i][1]])
        end = (int(img1.shape[1] + corners1[i][0]+ pt2[0]), int(corners1[i][1]+pt2[1]))
        #end = (int( corners1[i][0] + pt2[0]), int(corners1[i][1] + pt2[1]))
        print(end)
        cv2.line(img_draw_matches, tuple([int(j) for j in corners1[i]]), end, random_color(), 2)
    out = "rangetest/kdata/corr-point-more-flownet2.png"
