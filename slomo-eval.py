...
def interpolate_batch(frames, factor,done,output):
    ...
    writeFlow('%06d.flo'%(done), f01)
    writeFlow('%06d-inverse.flo'%(done) , f10)
...
def convert_video(source, dest, factor, batch_size=10, output_format='mp4v', output_fps=30):
    ...
    while True:
        ...
        intermediate_frames = interpolate_batch(batch, factor,done,dest)
